...
- **Layout**: `_includes/support.liquid` — a trimmed copy of `guide.liquid` for non-guide content (these pages, and any future FAQ/troubleshooting docs). Keeps the head/scripts, logo header, TOC, and the "Last modified" badge; drops the Version/Estimated Time/Complexity badges, which are assembly-guide-only metadata.
