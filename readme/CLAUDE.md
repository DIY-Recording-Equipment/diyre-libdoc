...
- `docs/contact/index.md`, `docs/contact/troubleshooting.md`, `docs/contact/parts.md`, `docs/contact/thanks.md` — the contact/support pages (general contact form, troubleshooting-request form, missing/replacement-parts request, and the shared post-submit confirmation page). None of these are assembly guides, so none use `layout: guide.liquid` — see "Support / contact form" below.
- `docs/troubleshooting/`, `docs/mods/`, `docs/shipping-and-returns/` — FAQ/reference articles migrated from the old HelpScout-based support.diy.re docs site (source export was dropped in a since-deleted `docs_articles/` staging folder). All use `layout: support.liquid` with a `date:` frontmatter key (**unquoted** `YYYY-MM-DD` — quoting it as a string triggers a `datePrefixText` filter quirk that double-prints "Last modified") set from the original article's `updated_at`, so the "Last modified" badge reflects real content history instead of the migration commit date. Where multiple old articles covered the same product's troubleshooting (e.g. EQP5 had separate "no signal" and "not unity gain" articles), they were combined into one `[Product] Troubleshooting` page with an `##` section per original topic — check for this pattern before adding a new per-symptom troubleshooting article for a product that already has one. `docs/troubleshooting/` is further split into `general/` and `product/` subfolders, each with its own `index.md` (`eleventyNavigation.parent: "Troubleshooting"`) — put a new article under whichever applies. Internal links between all these pages use root-relative `/docs/...` paths (e.g. `/docs/troubleshooting/general/slug/`); `docs/index.md`, `docs/troubleshooting/index.md`, `docs/mods/index.md`, and `docs/shipping-and-returns/index.md` are real listing/landing pages with their own `eleventyNavigation` entries, so each category is browsable, not just linkable.
...
There are multiple self-hosted contact/request forms, all sharing one PHP backend — separate from the older `{% feedbackForm %}` (post-build feedback, posts to the third-party FormSubmit.co, no attachments). Each emails `support@diyrecordingequipment.com` directly (with photo attachments where the form has them), via a plain PHP backend rather than a Node/serverless function, because Eleventy only ever outputs static files and this site has no other server runtime.

- **`/docs/contact/`** (`docs/contact/index.md`) — the general-purpose form (name, email, a visible "Subject" field named `_subject` directly, message) for presales questions and anything that doesn't fit the two specialized forms below. Above the form it links out to those two.
- **`/docs/contact/troubleshooting/`** (`docs/contact/troubleshooting.md`) — the troubleshooting-request form (kit/product, order #, issue description, test setup, what's been tried, up to 5 photos). Builds its `_subject` client-side from kit/product + order # right before submit, falling back to the hidden field's static default if JS is disabled.
- **`/docs/contact/parts/`** (`docs/contact/parts.md`) — missing/damaged-parts request form (name, email, kit, order #, a required radio for missing-vs-damaged, and a text box listing the parts needed). Explains up front that a shipped replacement gets a follow-up invoice, typically $5 + shipping.
- **`/docs/contact/thanks/`** (`docs/contact/thanks.md`) — the shared post-submit confirmation page all three forms redirect to. `eleventyExcludeFromCollections: true` keeps it out of nav/TOC. Its permalink must stay `docs/contact/thanks/index.html` — it was previously set to the bare `contact/thanks/index.html`, which silently built the page one directory up from where every link and the PHP handler expected it; `.htaccess` now 301s the old `/contact/`, `/contact/thanks/`, and `/contact-success` paths to their `/docs/contact/...` equivalents for anyone with the old URLs bookmarked or emailed.
- **Layouts**: all four pages above use `_includes/support.liquid` — a trimmed copy of `guide.liquid` for non-guide content. Keeps the head/scripts, logo header, TOC, and the "Last modified" badge; drops the Version/Estimated Time/Complexity badges, which are assembly-guide-only metadata. Also used by the migrated `docs/troubleshooting/`/`docs/mods/`/`docs/shipping-and-returns/` articles (where "Last modified" is meaningful content history). `_includes/page.liquid` is a near-identical copy minus the "Last modified" badge — it was used by the contact pages before they moved to `support.liquid`, and as of this writing nothing in the repo uses it; confirm it's still genuinely orphaned before deleting it, and if it does get revived, note it's a hand-kept fork of `guide.liquid` with no shared partial, so a header/footer change needs applying to all three files.
- **Form markup**: written directly as raw HTML in each page's markdown (no shortcode) — there's no shared `feedbackForm`-style fragment because each of these forms has its own field set. Every such form must: POST to `/support-form-handler.php` (add `enctype="multipart/form-data"` if it has a file input), include a field named `_subject` for the email subject, and include the honeypot markup pattern used on the existing forms (`class="hp-field"` wrapper + `tabindex="-1"` input named `website`) for spam deterrence — there's no CAPTCHA in this pipeline.
- **Character encoding**: `support-form-handler.php` sets `$mail->CharSet = 'UTF-8';` on the PHPMailer instance — without it, PHPMailer defaults to ISO-8859-1 and any non-ASCII byte in the subject or body (an em dash, an accented name) comes through as mojibake in the recipient's inbox.
