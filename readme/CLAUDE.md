...
- `contact.md`, `troubleshooting-support.md`, `missing-parts.md`, `contact-thanks.md` — the contact/support pages (general contact form, troubleshooting-request form, missing/replacement-parts request, and the shared post-submit confirmation page). None of these are assembly guides, so none use `layout: guide.liquid` (see "Support / contact form" below for which of `support.liquid`/`page.liquid` each uses).
- `troubleshooting/`, `mods/`, `shipping-and-returns/` — FAQ/reference articles migrated from the old HelpScout-based support.diy.re docs site (source export was dropped in a since-deleted `docs_articles/` staging folder). All use `layout: support.liquid` with a `date:` frontmatter key (**unquoted** `YYYY-MM-DD` — quoting it as a string triggers a `datePrefixText` filter quirk that double-prints "Last modified") set from the original article's `updated_at`, so the "Last modified" badge reflects real content history instead of the migration commit date. Where multiple old articles covered the same product's troubleshooting (e.g. EQP5 had separate "no signal" and "not unity gain" articles), they were combined into one `[Product] Troubleshooting` page with an `##` section per original topic — check for this pattern before adding a new per-symptom troubleshooting article for a product that already has one. Internal links between these pages use root-relative paths (`/troubleshooting/slug/`); there's no `/troubleshooting/`, `/mods/`, or `/shipping-and-returns/` index/listing page yet, so these are only reachable via direct links from other pages, not browsable as a category.
...
- **`/docs/contact/`** (`contact.md`) — the general-purpose form (name, email, subject, question) for presales questions and anything that doesn't fit the two specialized forms below. Above the form it links out to those two.
- **`/troubleshooting-support/`** (`troubleshooting-support.md`) — the troubleshooting-request form (kit/product, order #, issue description, test setup, what's been tried, photos). Was originally built at `/docs/contact/` and moved here once the general contact form was added.
- **`/missing-parts/`** (`missing-parts.md`) — missing/damaged-parts request form (name, email, kit, order #, a required radio for missing-vs-damaged, and a text box listing the parts needed). Explains up front that a shipped replacement gets a follow-up invoice, typically $5 + shipping.
- **Layouts**: `_includes/support.liquid` — a trimmed copy of `guide.liquid` for non-guide content. Keeps the head/scripts, logo header, TOC, and the "Last modified" badge; drops the Version/Estimated Time/Complexity badges, which are assembly-guide-only metadata. Used by `troubleshooting-support.md`, `missing-parts.md`, and the migrated `troubleshooting/`/`mods/`/`shipping-and-returns/` articles (where "Last modified" is meaningful content history). `_includes/page.liquid` is the same file minus the "Last modified" badge entirely — used by `contact.md` and `contact-thanks.md`, where a modified date isn't meaningful. Both are trimmed copies of `guide.liquid` kept in sync by hand; there's no shared partial between them, so a header/footer change usually needs applying to all three files.
- **Form markup**: written directly as raw HTML in each page's markdown (no shortcode) — there's no shared `feedbackForm`-style fragment because each of these forms has its own field set. Every such form must: POST to `/support-form-handler.php` (add `enctype="multipart/form-data"` if it has a file input), include a field named `_subject` for the email subject (either a hidden field with a fixed/computed value, as `troubleshooting-support.md` builds client-side from kit + order #, or the visible user-facing "Subject" field itself, as `contact.md` does by naming that input `_subject` directly), and include the honeypot markup pattern used on the existing forms (`class="hp-field"` wrapper + `tabindex="-1"` input named `website`) for spam deterrence — there's no CAPTCHA in this pipeline.
